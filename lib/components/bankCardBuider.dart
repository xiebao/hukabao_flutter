...
class CardDataItem extends StatelessWidget {
  ...
  static const double height = 190.0;
  static const double width = 315.0;
...
//                "images/bank/${data.cardIcon!='' ? data.cardIcon : 'unionpay'}.png",
                "images/bank/unionpay.png",
              ),height: 30,width: 40, ),
/*          CircleAvatar(
          ...
          subtitle: data.cardType=='2'? Text('单次限 ${data.singleQuota}，单日限 ${data.dayQuota}'):Text(''),
        );
    //${data.cardIcon}
  }
...
//        color: subcolor,
  ...
  Widget _buildContainer(String text,{bool sml=false}) {
    return new Container(
        margin: const EdgeInsets.only(left: 30.0, right: 30, top: 12),
        child: new Text(
          text,
          style: new TextStyle(
              fontSize: sml?12:15.0,
