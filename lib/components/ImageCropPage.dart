...
//    FormData _f = await createForm(image);
//    print(_f);

    DialogUtils.showToastDialog(context,  '正在更新头像');
    await HttpUtils.apipost(context, 'User/userAvatar',formData,
            (response) {
          print(response);
          if (response["error_code"].toString() == '1') {
            DialogUtils.showToastDialog(context,  '头像更新成功');
//            Navigator.of(context).pop(true);
            Navigator.pop(context);
          }else
            {
              DialogUtils.showToastDialog(context,  '头像更新失败');
